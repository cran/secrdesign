...
This document is a technical guide to the simulation aspects of **secrdesign**. 
Functions that do not rely on simulation are described in [secrdesign-tools.pdf].

We assume an understanding of estimator properties such as bias, precision, and
confidence interval coverage, and the use of Monte Carlo simulation to
predict the performance of different sampling designs. Using **secrdesign** can 
be daunting because it allows for many different combinations of data generation, 
model fitting and summary statistics. Several examples are given to indicate the
range of possibilities.

# Introduction

When designing a study you should have in mind --
...
```{r Ex1, cache = TRUE, eval = TRUE, warning = FALSE}
library(secrdesign)
...
The initial summary includes the expected number of individuals (En) and recaptures (Er), and an approximate rule-of-thumb relative standard error for estimated density (rotRSE). `scenarioSummary` approximates the default detection function (HN) by a hazard-based detection function (HHN) with a warning that is not shown here. See `?scenarioSummary` and [secrdesign-tools.pdf] for further details.
...
The use of `multisession`, `ncores` and `byscenario` is discussed under 
...
The default (builtin) `extractfn` ([Appendix 1](#appendix1)) behaves appropriately for either
...
`derived` or `region.N` (see [Extracting estimate tables from fitted models](#extracting-estimate-tables-from-fitted-models)).
...
The methods `predict`, `coef`, `derived' and `region.N' for the 'fittedmodels'
class are provided to extract estimates of 'real' parameters from each fitted model. These are direct analogues of `predict`, `coef`, `derived` and `region.N' in **secr**. Here, they apply across all
replicates of all scenarios and return an object of class `estimatetables`. 
...
for `region.N`) and columns to the respective estimates,
standard errors, and confidence limits (with some variations).

The \dots argument of the functions `predict`, `coef`, `derived` and `region.N` lets you pass arguments such as `alpha` to the corresponding **secr** function (e.g., `predict.secr` or `derived.secr`).
...
Given tabular output from `predict()` or `derived()`,
...
plot(stats1, type = "CI")
```
```{r plotpng, echo=FALSE, eval=makepng}
...
separate scenarios on separate cores (given the default 
`byscenario = TRUE`). This uses the R package
...
in how long they take to run: the system waits for the slowest. Setting 
`byscenario = FALSE` provides a way around this: replicates within a 
scenario are divided among cores, instead of all replicates going to one core.
...
A full demonstration is given in Appendix 2 ([Non-uniform possums](#non-uniform-possums)).
...
sims3 <- fit.models(raw3, fit.args = list(list(model = g0~1),
    list(model = g0~T)), fit = TRUE, ncores = 4, byscenario = FALSE)
summary(sims3)
```
...
An example is given in Appendix 2 ([Grouped populations](#grouped-populations)).
...
# Appendix 1. Default extractfn for `run.scenarios`. {#appendix1}
...
summary(sims4)
```
```{r Ex4Plt, eval = FALSE, fig.width = 8, fig.height = 8}
...
```
```{r Ex4png, eval=makepng, echo=FALSE}
...
sims6a <- run.scenarios (1, scen6, traps(possumCH), possummask,
    ...
    extractfn = identity)
```
```{r Ex6apl, eval = TRUE, fig.width=8.5, fig.height = 4}
...
have the package **secrlinear**. Some warnings and messages are suppressed.
...
sims8 <- run.scenarios(20, scen8, trapset = list(grid), fit = TRUE, extractfn = exfn,
                       ...
                       maskset = list(mask), ncores = 4, byscenario = FALSE)
```
... and check the summary output for density ('D') and sex ratio (proportion female 'pmix') without repeating the header information:
```{r Ex8Sum, cache = TRUE}
